...
这个定义相当于其他语言中的类的数据部分，但 **Rust 的结构体本身不包含方法**。方法和函数是在后续的 `impl` 块中定义的，不像一般面向对象语言那样把数据和行为放在同一个 class 内。
...
在官方 Rust 书中也有类似 IP 地址枚举的例子：可以直接将数据附加到枚举变体，而无需单独结构体来组合种类和地址数据。([kaisery.github.io][4])
...
此外，Rust 中广泛使用 `if let` 和 `while let` 等配合模式匹配使用的语法，简化只关心某个变体的情况（参考进一步阅读）。([Sonia33][5])
